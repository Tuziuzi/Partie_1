...
B.append('<div class="karte warn"><h2>Hochenergie — zwei Regeln, eine Entscheidung</h2>'
 '<p>Der <code>shipyard-designer</code> ist auf <b>v5.22-HE</b> gepatcht und trennt jetzt sauber, '
 'was ich vorher vermengt hatte:</p><div class="wrap"><table>'
 '<tr><th></th><th>Spielregel (<code>construction.md</code>)</th><th>Kalkulator (<code>payload_catalog.md</code>)</th></tr>'
 '<tr><th>Auslöser</th><td>System zieht <b>≥ ~50 kW</b></td><td>np &gt; 0 (Voreinstellung an)</td></tr>'
 '<tr><th>Wirkung</th><td><b>Δv ÷ 3 und 3× Gewicht</b></td><td>Nutzlast <b>×3</b>, Δv unberührt</td></tr>'
 '</table></div>'
 '<p>SKILL.md Schritt 4, wörtlich: <i>«Never present the calculator\'s ×3 as "the high-energy '
 'penalty" — it is a payload guard coupled to np, nothing more.»</i></p>'
 '<div class="wrap"><table><tr><th>Schiff</th><th class="n">Spitzenlast</th><th>Spielregel</th>'
 '<th>Δv ÷ 3</th><th>gewählter Zustand</th><th class="n">Faktor</th></tr>'
 '<tr><td>Feldzeichen</td><td class="n">3,60 kW</td><td class="ok">greift nicht</td>'
 '<td class="ok">nein</td><td>Voreinstellung ×3</td><td class="n">15</td></tr>'
 '<tr><td>Himmelsauge</td><td class="n">11,00 kW</td><td class="ok">greift nicht</td>'
 '<td class="ok">nein</td><td><code>ctxNoHE</code> (cm × np)</td><td class="n">2</td></tr>'
 '<tr><td>Himmelsbrücke</td><td class="n">17,70 kW</td><td class="ok">greift nicht</td>'
 '<td class="ok">nein</td><td>Voreinstellung ×3</td><td class="n">18</td></tr></table></div>'
 '<h3>RULES-GAP — Vorlage an den Tisch [S]</h3>'
 '<p>Schritt 2 verlangt sie ausdrücklich: unterhalb 50&nbsp;kW kennt die Spielregel <b>gar keine '
 'Strafe</b>, der Kalkulator bietet aber nur zwei Zustände. Gewählt ist je Schiff der <b>nähere</b>; '
 'die Differenz ist eine Setzung <b>[S]</b>. Drei Optionen:</p><div class="wrap"><table>'
 '<tr><th>Option</th><th class="n">Feldzeichen</th><th class="n">Himmelsauge</th>'
 '<th class="n">Himmelsbrücke</th><th>Folge</th></tr>'
 '<tr><td><b>(a)</b> Kalkulator ×3</td><td class="n">15 ✓</td><td class="n">6</td>'
 '<td class="n">18 ✓</td><td>Aufklärer wird 7,5&nbsp;t → Werft 139,6&nbsp;t, <b>sprengt die '
 'Kapazität</b></td></tr>'
 '<tr><td><b>(b)</b> <code>ctxNoHE</code> cm × np</td><td class="n">25</td><td class="n">2 ✓</td>'
 '<td class="n">36</td><td>Feldzeichen 2,39&nbsp;t und Himmelsbrücke 7,63&nbsp;t — beide über Anker</td></tr>'
 '<tr><td><b>(c)</b> Hausregel flach [S]</td><td class="n">5</td><td class="n">2</td>'
 '<td class="n">6</td><td>alle passen, aber die Schiffe werden sehr billig</td></tr>'
 '<tr><td><b>gewählt: je Schiff der nähere</b></td><td class="n fett">15</td>'
 '<td class="n fett">2</td><td class="n fett">18</td>'
 '<td class="ok">alle drei treffen ihren Anker auf 0,0000&nbsp;%</td></tr></table></div>'
 '<p class="q">✓ = im Anker. Die gemischte Wahl ist keine Rosinenpickerei, sondern die Vorschrift: '
 '«pick the state closest to the rule» — und die Regel kennt unter 50&nbsp;kW keine Strafe, also ist '
 'der kleinere Faktor der nähere. Bei np&nbsp;5 und np&nbsp;6 ist das ×3, bei np&nbsp;1 ist es '
 '<code>ctxNoHE</code>. Der Tisch kann stattdessen (a), (b) oder (c) einheitlich festlegen.</p></div>')
